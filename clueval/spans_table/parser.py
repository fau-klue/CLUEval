...
class BioToSpanParser:
    ...
    def extract_tokens_from_iob(self, n_tag_columns=1, doc_token_id_column: int=None, domain_column: int=None, doc_id_column: int=None):
        """
        :param n_tag_columns:
    ...
    def extract_spans_from_iob(self, tag_column=1, doc_id_column: int = None):
        """
        ...
        with open(self.path_to_file, "r", encoding="utf-8") as in_f:
            ...
            for i, line in enumerate(lines):
                ...
                current_line = line.strip()
                if "newdoc id" in current_line:
                    current_doc_id = current_line.split("=")[1].strip()
                else:
                    if doc_id_column and current_line.strip() != "":
                        doc_id = current_line.strip().split("\t")[doc_id_column].strip()
